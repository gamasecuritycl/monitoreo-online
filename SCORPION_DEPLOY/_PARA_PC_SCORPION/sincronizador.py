...
def sincronizar_zonas():
    ...
    Mismo formato que el sincronizador v5.0 probado: cuenta = nombre del archivo MDB,
    tabla 'ZONAS' con columnas NUMERO, DISPOSITIVOS, AREA, y reemplazo completo de la fila."""
    ...
    for archivo in archivos:
        # La cuenta SIEMPRE es el nombre del archivo (ej: C7C9.MDB -> C7C9), igual que en v5.0
        ...
        try:
            ...
            # Mismas columnas que v5.0: NUMERO, DISPOSITIVOS, AREA (posiciones 0,1,2)
            try:
                cursor.execute(f'SELECT NUMERO, DISPOSITIVOS, AREA FROM [{tabla}]')
            except Exception:
            ...
            for fila in filas:
                ...
                disp  = get_val(fila, ['DISPOSITIVOS', 'DISPOSITIVO', 'DEVICE', 'TIPO', 'TIPO_DISPOSITIVO', 'DESCRIPCION'], 1)
                area  = get_val(fila, ['AREA', 'UBICACION', 'DESCRIPCION_AREA', 'LUGAR', 'SECTOR'], 2)
                if not num:
                    continue
                num_norm = num.zfill(2) if num.isdigit() else num
                lista_zonas.append({
                    "numero": num_norm,
                    "dispositivo": disp,
                    "area": area
    ...
    try:
        ...
        # Reemplazo completo: borrar fila(s) ZONAS anterior(es) e insertar una sola, como v5.0
        try:
        ...
        print(f"  [ZONAS SUCCESS] Zonificación de {len(zonas_map)} abonados sincronizada desde {carpeta_zonas}.")
        return True
    except Exception as e:
