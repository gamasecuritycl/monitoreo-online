...
export async function POST(req: Request) {
  try {
    ...
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
        <div style="background-color: #000080; padding: 20px; text-align: center;">
          <img src="https://dashboard-ten-self-68.vercel.app/logo.jpg" alt="Gama Security" style="max-height: 80px;" />
        </div>
    ...
    const data = await resend.emails.send({
      ...
      to: destinatarios,
      subject: `Notificación de ${tipo_evento.toUpperCase()} - Cuenta ${cuenta}`,
      html: htmlContent,
    })
