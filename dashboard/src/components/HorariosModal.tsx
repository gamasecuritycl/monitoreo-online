      ...
      >
        ...
        <div className="bg-[#000080] text-white font-bold px-2 py-1 flex justify-between items-center select-none shrink-0 h-6">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] tracking-wide">Scorpion - Control de Horarios de Apertura y Cierre</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 p-1 flex flex-col md:flex-row gap-2 overflow-hidden min-h-0">
          ...
          {/* LADO IZQUIERDO: Directorio de Abonados (320px) */}
          <div className="w-full md:w-[320px] bg-[#d4d0c8] border-2 border-t-[#808080] border-l-[#808080] border-b-white border-r-white p-1.5 flex flex-col gap-1.5 shrink-0 overflow-hidden">
            <div className="bg-[#808080] text-white font-bold px-1.5 py-0.5 text-[10px] uppercase border border-t-black border-l-black border-b-white border-r-white flex justify-between items-center">
              <span>DIRECTORIO ABONADOS</span>
              <span className="text-[9px] font-mono opacity-90">{listaAbonados.length} Total</span>
            </div>
            ...
            <input
              ...
              className="w-full bg-[#ffffd0] border border-t-gray-700 border-l-gray-700 border-b-white border-r-white px-1.5 py-1 text-[11px] font-bold text-blue-900 focus:outline-none placeholder-gray-500"
            />
            ...
            <div className="flex-1 bg-white border border-t-gray-700 border-l-gray-700 border-b-white border-r-white overflow-y-auto">
              <table className="w-full border-collapse text-[10px] text-left">
                <thead className="bg-[#b0b0b0] border-b border-gray-400 sticky top-0 font-bold z-10">
                  <tr>
                    <th className="p-1 border-r border-gray-400 w-14">CTA</th>
                    <th className="p-1">NOMBRE ABONADO</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-300 font-bold">
                  {listaFiltrada.map(a => (
                    ...
                    >
                      <td className="p-1 border-r border-gray-300 font-mono text-center">
                        {a.cuenta}
                      </td>
                      <td className="p-1 truncate max-w-[200px]" title={a.nombre}>
                        {a.nombre}
                      </td>
                    </tr>
                  ))}
                  {listaFiltrada.length === 0 && (
                    <tr>
                      <td colSpan={2} className="p-4 text-center text-gray-400 italic">No se encontraron abonados</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="flex-1 bg-[#d4d0c8] border-2 border-t-[#808080] border-l-[#808080] border-b-white border-r-white p-2 flex flex-col gap-2 overflow-y-auto min-h-0">
            
            {/* Header del Abonado Seleccionado */}
            <div className="bg-[#e6f0fa] border border-blue-400 p-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 rounded-xs">
              <div>
                <div className="flex items-center gap-2">
                  <span className="bg-[#000080] text-white font-mono font-bold px-2 py-0.5 text-xs rounded-xs">
                    {cuentaActiva}
                  </span>
              ...
              <div className="flex flex-wrap gap-1 items-center">
                <span className="text-[9px] font-bold text-gray-600 uppercase">Plantillas:</span>
                <button
                  type="button"
            ...
            <div className="flex-1 bg-white border border-t-gray-700 border-l-gray-700 border-b-white border-r-white overflow-y-auto">
              <table className="w-full border-collapse text-[11px] text-left">
                <thead className="bg-[#b0b0b0] border-b border-gray-400 font-bold sticky top-0">
                  <tr>
                    <th className="p-1.5 border-r border-gray-400 w-28">DÍA</th>
                    <th className="p-1.5 border-r border-gray-400 w-28 text-center">ESTADO</th>
                    <th className="p-1.5 border-r border-gray-400 w-36 text-center">HORA APERTURA</th>
                    <th className="p-1.5 border-r border-gray-400 w-36 text-center">HORA CIERRE</th>
                    <th className="p-1.5 border-r border-gray-400 w-32 text-center">TOLERANCIA</th>
                    <th className="p-1.5">RESUMEN OPERATIVO</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-300 font-bold">
                  {dias.map((d, idx) => (
                    <tr key={d.dia} className={`hover:bg-blue-50 ${!d.habilitado ? 'bg-gray-100 text-gray-500' : 'text-gray-900'}`}>
                      {/* Día */}
                      <td className="p-1.5 border-r border-gray-300 font-extrabold flex items-center gap-1.5">
                        <span className={`w-2 h-2 rounded-full ${d.habilitado ? 'bg-green-600' : 'bg-gray-400'}`} />
                        <span>{d.label}</span>
                      </td>
                      ...
                      <td className="p-1.5 border-r border-gray-300 text-center">
                        <button
                          ...
                          className={`px-2 py-0.5 text-[9px] font-bold rounded-xs cursor-pointer ${
                            d.habilitado
                      ...
                      <td className="p-1.5 border-r border-gray-300 text-center">
                        <input
                          ...
                          className={`px-1 py-0.5 border border-gray-400 font-mono text-center text-xs font-bold ${
                      ...
                      <td className="p-1.5 border-r border-gray-300 text-center">
                        <select
                          ...
                          className={`px-1 py-0.5 border border-gray-400 font-bold text-[10px] ${
                      ...
                      <td className="p-1.5 text-[10px] text-gray-600">
                        {d.habilitado ? (
                          <span>
                            Apertura esperada <strong className="text-blue-900 font-mono">{d.apertura}</strong> | Cierre límite <strong className="text-blue-900 font-mono">{d.cierre}</strong> (Tol. {d.toleranciaMin}m)
                          </span>
                        ) : (
            ...
            <div className="bg-[#e8e4dc] border border-gray-400 p-2 space-y-2 rounded-xs text-[10px]">
              <div className="font-bold text-gray-800 uppercase tracking-wider flex items-center gap-1">
              ...
              {notificarNoCierre && (
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-1 border-t border-gray-300">
                  <span className="font-bold text-emerald-900 w-44">WhatsApp de Aviso No Cierre:</span>
                  <input
                    type="text"
                    placeholder="Ej: 56912345678"
                    value={telefonoWhatsApp}
                    onChange={(e) => setTelefonoWhatsApp(e.target.value)}
                    className="flex-1 bg-white border border-gray-400 px-2 py-0.5 font-mono text-[11px] font-bold text-black"
                  />
            ...
            {/* FEEDBACK STATUS */}
            {statusMsg && (
              <div className="text-center font-bold text-[11px] text-blue-900 py-0.5 animate-pulse">
                {statusMsg}
              </div>
            )}

            {/* BOTONES DE ACCIÓN INFERIOR */}
            <div className="flex justify-between items-center gap-2 pt-1 border-t border-gray-400">
              <div className="text-[10px] text-gray-600">
                Abonado activo: <strong className="font-mono text-blue-900">[{cuentaActiva}]</strong>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={onClose}
                  className="bg-[#d4d0c8] border border-t-white border-l-white border-b-black border-r-black px-4 py-1 text-xs font-bold text-black hover:bg-[#e8e8e8] cursor-pointer"
                >
                  Cerrar
                </button>
                ...
                <button
                  ...
                  disabled={guardando}
                  className="bg-[#000080] border border-t-white border-l-white border-b-black border-r-black px-5 py-1 text-xs font-bold text-white hover:bg-blue-900 active:border-t-black active:border-l-black active:border-b-white active:border-r-white cursor-pointer shadow-md disabled:opacity-50"
                >
